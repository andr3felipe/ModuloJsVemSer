...
var start = true;
while (start) {
  const firstNumber = getNumber("Insira o primeiro n\xFAmero: ");
  const secondNumber = getNumber("Insira o segundo n\xFAmero: ");
  const operationType = getOperation(
    `Insira o tipo de opera\xE7\xE3o(+, -, * ou /): `
  );
  console.log(
...
function getNumber(message) {
  ...
  while (parseInt(input) !== Number(input)) {
    console.log("---> Por favor, insira um n\xFAmero v\xE1lido <---.\n");
...
function getOperation(message) {
  ...
  while (!["+", "-", "*", "/"].includes(input)) {
    console.log("---> Por favor, insira uma opera\xE7\xE3o v\xE1lida <---.\n");
    input = prompt(message);
  }
  return input;
}
function operation({
  ...
  if (operationType === "+") {
    return `O resultado da soma dos n\xFAmeros \xE9: ${Number(num1) + Number(num2)}`;
  }
  if (operationType === "-") {
    return `O resultado da subtra\xE7\xE3o dos n\xFAmeros \xE9: ${Number(num1) - Number(num2)}`;
  }
  if (operationType === "*") {
    return `O resultado da multiplica\xE7\xE3o dos n\xFAmeros \xE9: ${Number(num1) * Number(num2)}`;
  }
  if (operationType === "/") {
    if (num2 === 0)
      return "N\xE3o \xE9 poss\xEDvel dividir por 0.";
    return `O resultado da divis\xE3o dos n\xFAmeros \xE9: ${Number(num1) / Number(num2)}`;
  }
}
