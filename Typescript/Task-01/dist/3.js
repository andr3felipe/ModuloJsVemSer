...
function getName(message) {
  ...
  while (typeof input !== "string" || !regex.test(input)) {
    console.log("---> Por favor, insira um nome v\xE1lido. <---\n");
    input = prompt(message);
  }
  return input;
}
function calculateDifferenceInAge({
  ...
  if (Number(firstPersonAge) > Number(secondPersonAge)) {
    return `A primeira pessoa (${firstPersonName}) \xE9 mais velha que a segunda por ${firstPersonAge - secondPersonAge} ano(s).`;
  } else if (Number(firstPersonAge) < Number(secondPersonAge)) {
    return `A segunda pessoa (${secondPersonName}) \xE9 mais velha que a primeira por ${secondPersonAge - firstPersonAge} ano(s).`;
  } else {
    return `Ambas as pessoas (${firstPersonName} e ${secondPersonName}) t\xEAm a mesma idade.`;
  }
}
